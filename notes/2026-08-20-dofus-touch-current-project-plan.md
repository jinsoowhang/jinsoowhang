...
**Architecture:** This is a documentation-only change to the existing flat Markdown list. A shell assertion defines and verifies the exact new line and its position; project memory and session notes record the decision and verification without introducing new tooling.

**Tech Stack:** Markdown, POSIX shell, ripgrep, Git
...
- [ ] **Step 1: Run the failing position assertion**
...
test "$(sed -n '7p' README.md)" = "$expected_entry"
```

Expected: FAIL with exit status 1 because line 7 is currently the `world-cup-tickets` entry.
...
- [ ] **Step 3: Run the position assertion and exact-count check**
...
- Confirmed the entry appears exactly once on line 7, directly after `skills`.
...
test "$(rg -Fxc -- "$expected_entry" README.md)" -eq 1
git diff --check
git diff -- README.md MEMORY.md 'notes/Session Notes 2026-08-20.md'
```

Expected: exit status 0; the diff contains only the new README entry, project memory, and session notes.

- [ ] **Step 6: Commit the implementation atomically**

```bash
git add README.md MEMORY.md 'notes/Session Notes 2026-08-20.md'
